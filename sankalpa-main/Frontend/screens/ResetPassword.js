...
const ResetPwd = ({ route }) => {
    ...
    const checkEmailInApi = async () => {
        const endpoints = [
            'http://192.168.1.100:8000/api/studentby',
            'http://192.168.1.100:8000/api/teachersby'
        ];
        ...
        if (userId) {
            try {
                const endpoint = emailEndpoint.includes('teachersby')
                    ? 'http://192.168.1.100:8000/api/teacher/update'
                    : 'http://192.168.1.100:8000/api/student/update';
